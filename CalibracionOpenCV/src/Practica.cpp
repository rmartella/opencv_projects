...
int main(int argc, char* argv[]) {
	...
	CvCapture* capture = cvCaptureFromFile("http://192.168.1.89:8080/shot.jpg");
	assert(capture);
...
//Handle pause/unpause and ESC
		...
		capture = cvCaptureFromFile("http://192.168.1.89:8080/shot.jpg");
...
//
	for (int i = 0; i < successes * board_n; ++i) {
		...
		CV_MAT_ELEM( *image_points2, float,i,1) = CV_MAT_ELEM(*image_points,
				float, i, 1);
		CV_MAT_ELEM(*object_points2, float, i, 0) = CV_MAT_ELEM(*object_points,
				float, i, 0);
		CV_MAT_ELEM( *object_points2, float, i, 1) = CV_MAT_ELEM(*object_points,
				float, i, 1);
		CV_MAT_ELEM( *object_points2, float, i, 2) = CV_MAT_ELEM(*object_points,
				float, i, 2);
	}
