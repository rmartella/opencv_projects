...
void XN_CALLBACK_TYPE onNewUser(xn::UserGenerator& generator, XnUserID nId,
		void* pCookie) {
...
void initKinect(bool initImage, bool initDepth, bool initIR, bool initUser) {
	...
	if (initImage) {
		...
		cout << "imageGen.SetMapOutputMode : " << xnGetStatusString(nRetVal)
				<< endl;
		nRetVal = imageGen.GetMirrorCap().SetMirror(true);
		cout << "imageGen.GetMirrorCap().SetMirror : "
	...
	if (initDepth) {
		...
		cout << "depthGen.SetMapOutputMode : " << xnGetStatusString(nRetVal)
				<< endl;
		nRetVal = depthGen.GetMirrorCap().SetMirror(true);
		cout << "depthGen.GetMirrorCap().SetMirror : "
				<< xnGetStatusString(nRetVal) << endl;
	}
	...
	if (initIR) {
		...
		cout << "irGen.SetMapOutputMode : " << xnGetStatusString(nRetVal)
				<< endl;
	}
	...
	cout << "context.StartGeneratingAll : " << xnGetStatusString(nRetVal)
			<< endl;
}

float getJointImgCoordinates(const SkeletonCapability &skeletonCapability,
		const XnUserID userId, const XnSkeletonJoint skeletonJoint, float *v) {
	XnVector3D projective;
	XnSkeletonJointPosition skeletonJointPosition;
	skeletonCapability.GetSkeletonJointPosition(userId, skeletonJoint,
			skeletonJointPosition);

	depthGen.ConvertRealWorldToProjective(1, &skeletonJointPosition.position,
			&projective);
...
bool getHandContour(const Mat &depthMat, const float *v,
...
void detectFingerTips(const vector<Point> &handContour,
		vector<Point> &fingerTips, Mat *debugFrame = NULL) {
	...
	for (int j = 0; j < hull.size(); j++) {
		...
		if (handContour[idx].y < upper)
			upper = handContour[idx].y;
		if (handContour[idx].y > lower)
			lower = handContour[idx].y;
	}
	...
	for (int j = 0; j < hull.size(); j++) {
		...
		if (angle < 1 && handContour[idx].y < cutoff) {
			...
			if (debugFrame) { // draw fingertips
				cv::circle(*debugFrame, handContour[idx], 10,
						debugFingerTipColor, -1);
			}
		}
	}
	...
	if (debugFrame) {
		...
		cv::line(*debugFrame, Point(0, cutoff), Point(640, cutoff),
				debugFingerTipColor);
		...
		for (int j = 0; j < handContour.size(); j++) {
			...
			if (j != 0) {
				cv::line(*debugFrame, handContour[j], handContour[j - 1],
						debugFingerTipColor);
			} else {
				cv::line(*debugFrame, handContour[0],
						handContour[handContour.size() - 1],
						debugFingerTipColor);
			}
		}
		...
		for (int j = 0; j < hull.size(); j++) {
			cv::circle(*debugFrame, handContour[hull[j]], 10,
					debugFingerTipColor, 3);
			/*if (j == 0) {
				cv::line(*debugFrame, handContour[hull[j]],
						handContour[hull[hull.size() - 1]],
						debugFingerTipColor);
			} else {
				cv::line(*debugFrame, handContour[hull[j]],
						handContour[hull[j - 1]], debugFingerTipColor);
			}*/
		}
	}
}
...
int main() {
	...
	while ((key = (char) waitKey(1)) != 27) {
		...
		// acquire bgr image
		{
			Mat mat(frameSize, CV_8UC3,
					(unsigned char*) imageGen.GetImageMap());
			cvtColor(mat, bgrMat, CV_RGB2BGR);
		}

		// acquire depth image
		{
			Mat mat(frameSize, CV_16UC1,
					(unsigned char*) depthGen.GetDepthMap());
			mat.copyTo(depthMat);
		...
		for (int i = 0; i < nUsers; i++) {
			...
			if (getJointImgCoordinates(skelCap, id, XN_SKEL_TORSO, t) == 1) {
				...
				circle(depthMatBgr, Point(t[0], t[1]), 10, Scalar(shade, 0, 0),
						-1);

				// right hand
				if ((getJointImgCoordinates(skelCap, id, XN_SKEL_RIGHT_HAND, rh)
				...
				&& (rh[1] < t[1]) /* user raises his hand */
					...
					circle(depthMatBgr, Point(rh[0], rh[1]), 10, color,
							thickness);
				...
				// left hand
				if ((getJointImgCoordinates(skelCap, id, XN_SKEL_LEFT_HAND, lh)
						== 1) && (lh[2] < t[2] - minHandExtension)
						&& (lh[1] < t[1]) /* user raises his hand */
						) {
					unsigned char shade = 255
							- (unsigned char) (lh[2] * 128.0f);
					...
					bool grasp = convexity(handContour) > grabConvexity;
					int thickness = grasp ? CV_FILLED : 3;
					circle(depthMatBgr, Point(lh[0], lh[1]), 10, color,
							thickness);
