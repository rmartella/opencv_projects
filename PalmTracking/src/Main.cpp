...
CvSeq* fingerseq = cvCreateSeq(CV_SEQ_ELTYPE_POINT, sizeof(CvSeq),
		sizeof(CvPoint), fingerstorage);
...
CvSeq* hull;
CvSeq* defect;
CvSeq* palm = cvCreateSeq(CV_SEQ_ELTYPE_POINT, sizeof(CvSeq), sizeof(CvPoint),
		palmstorage);
//int hullcount;//hull
...
int main() {
	...
	writer = cvCreateVideoWriter("palm_output2.avi",
			CV_FOURCC('M', 'J', 'P', 'G'), 15, cvSize(640, 480), 1);

	int i = 0;
	std::stringstream ss;
	ss << "ImageTest/" << "prueba2.png";
	capture = cvCaptureFromFile(ss.str().c_str());
	...
	while (1) {
		...
		for (int i = 0; i < frame->height; i++) //REPLACE ME
			...
			for (int j = 0; j < frame->width; j++) {
				...
				if (frame->imageData[i * frame->widthStep + (j * 3)] < 50
						|| frame->imageData[i * frame->widthStep + (j * 3) + 2]
								> 170) {
					mask->imageData[i * mask->width + j] = 255;
				} else
					mask->imageData[i * mask->width + j] = 0;
			}
		}
		...
		cvDrawContours(frame, contours, CV_RGB(100, 100, 100),
				CV_RGB(0, 255, 0), 1, 2, CV_AA, cvPoint(0, 0));
		...
		if (savepic) {
			int framenum = (int) cvGetCaptureProperty(capture,
	...
	for (i = 0; i < count; i++) {
		if ((tiplocation[i] - tiplocation[i - 1]) > 40) {
			if (fingertip[i].x >= 630 || fingertip[i].y >= 470) {
				cvCircle(frame, fingertip[i], 6, CV_RGB(50, 200, 250), -1, 8,
						0);
			} else {
...
void hand() {
	...
	if (palm->total <= 2) {
		...
		cvPutText(frame, "Error Palm Position!!", cvPoint(10, 50), &Font1,
				CV_RGB(255, 0, 0));
		//savepic = true;

		CvPoint *temp, *additional, *palmtemp;
		CvMemStorage* palm2storage = cvCreateMemStorage(0);
		CvSeq* palm2 = cvCreateSeq(CV_SEQ_ELTYPE_POINT, sizeof(CvSeq),
				sizeof(CvPoint), palm2storage);
		...
		for (int i = 0; i < palm->total; i++) {
			...
			for (int j = 1; j < contours->total; j++) {
				...
				if (temp->y == palmtemp->y && temp->x == palmtemp->x) {
					additional = (CvPoint*) cvGetSeqElem(contours,
							(int) (j + ((contours->total) / 2))
									% (contours->total));
					if (additional->y <= palmtemp->y)
						cvCircle(frame, *additional, 10, CV_RGB(0, 0, 255), -1,
								8, 0);
	...
	if (useavepalm) {
		...
		for (int i = 0; i < palm->total; i++) {
			...
			lengthtemp = sqrtf(
					(distemp.x * distemp.x) + (distemp.y * distemp.y));
			radius2 += lengthtemp;
		}

		radius2 = (int) (radius2 / palm->total);
		radius = ((0.5) * radius + (0.5) * radius2);
		mincirclecenter2.x = ((0.5) * mincirclecenter2.x
				+ (0.5) * avePalmCenter.x);
		mincirclecenter2.y = ((0.5) * mincirclecenter2.y
				+ (0.5) * avePalmCenter.y);
	}
	//////////////////////////////////////////////////////////////////////////////////////
	...
	cvCircle(frame, mincirclecenter2, cvRound(radius), CV_RGB(255, 0, 0), 2, 8,
			0);
	cvCircle(frame, mincirclecenter2, cvRound(radius * 1.2),
			CV_RGB(200, 100, 200), 1, 8, 0);
	...
	for (int i = 0; i < fingerseq->total; i++) {
		...
		fingerlength = sqrtf(
				(tiplength.x * tiplength.x) + (tiplength.y * tiplength.y));
