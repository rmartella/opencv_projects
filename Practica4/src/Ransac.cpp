...
void Ransac::ransacAlgorithm(XnPoint3D * points, int xI, int yI, int width, int height, int n,
		float t, int d, Plano * modelResult, bool inicio) {
	...
	for (int k = 0; k < n; k++) {
		if (bandera) {
			...
			int x = xI + rand() % (width - xI);
			int y = yI + rand() % (height - yI);
			for (; x < width; x++) {
				for (; y < height; y++) {
					inxP1 = y * width + x;
		...
		for (int i = 0; i < height * width; i++) {
			if (points[i].X != 0 && points[i].Y != 0 && points[i].Z != 0) {
				float distancia = plano->distanciaPunto(&points[i]);
				if (distancia < t) {
					model[consenso].X = points[i].X;
					model[consenso].Y = points[i].Y;
					model[consenso].Z = points[i].Z;
					consenso++;
